...
# אתחול שרת Flask מותאם פיקס לפורט של Render
app = Flask('')

@app.route('/')
def home():
    return "Chicago City Bot - Advanced Status System Active!"
...
async def update_fivem_status():
    ...
    # 2. משיכת נתונים מה-API הרשמי של FiveM
    ...
    embed = discord.Embed(
        ...
        description="הנתונים מתעדכנים באופן אוטומטי לחלוטין כל 90 שניות.",
        color=embed_color
    )
    
    embed.add_field(
        name="🎮 שרת המשחק (FiveM)",
        value=f"```properties\nStatus        : {status_text}\nPlayers       : {clients}/{max_clients}\nStaff In-Game : {staff_online_fivem}\n```",
        inline=False
    )
    embed.add_field(
    ...
    embed.set_footer(text="Chicago City Network • Auto Synchronized")
    embed.timestamp = discord.utils.utcnow()

    # 5. שליחה ועריכה אוטומטית (ללא צורך בפקודה ידנית)
    try:
...
async def on_message(message):
    if message.author.bot:
        return
    await bot.process_commands(message) # קריטי: מאפשר לפקודות לעבוד במקביל ל-Loop
