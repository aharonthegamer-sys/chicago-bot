...
# ========================================================
# 1. שרת FLASK הרמטי עבור RENDER
# ========================================================
app = Flask('')

@app.route('/')
def home():
    return "Chicago City Diamond Core Automation is Online!"
...
# ========================================================
# 2. קונפיגורציית ערוצים ורולים רשמית (CHICAGO CITY)
# ========================================================
SERVER_NAME = "Chicago City Roleplay"
GUILD_ID = 1483039214793789483

# קישור הלוגו הרשמי של שיקגו סיטי לפאנלים
LOGO_URL = "https://discordapp.com"

# ערוצי פנלים (אוטומציה מלאה)
STATUS_CHANNEL_ID = 1506965475270332476       # סרבר סטטוס
WELCOME_CHANNEL_ID = 1483039215032041530      # ברוכים הבאים
VERIFY_PANEL_CH = 1483039214793789489         # חדר אימות (Verify)
TICKET_PANEL_CH = 1483039219654852612         # חדר פתיחת טיקטים
...
class VerifyView(discord.ui.View):
    ...
    @discord.ui.button(label="🔑 אימות חשבון / VERIFY", style=discord.ButtonStyle.green, custom_id="verify_btn_v7_auto")
...
class TicketControlView(discord.ui.View):
    ...
    @discord.ui.button(label="🔒 סגור טיקט / Close", style=discord.ButtonStyle.danger, custom_id="btn_close_t_v7")
    ...
    @discord.ui.button(label="🙋‍♂️ קח טיפול / Claim", style=discord.ButtonStyle.success, custom_id="btn_claim_t_v7")
    async def claim_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        staff_role = interaction.guild.get_role(STAFF_ROLE_ID)
        if staff_role not in interaction.user.roles and not interaction.user.guild_permissions.administrator:
            return await interaction.response.send_message("❌ צוות בלבד מורשה ללחוץ!", ephemeral=True)
    ...
    @discord.ui.button(label="✏️ שינוי שם", style=discord.ButtonStyle.primary, custom_id="btn_rename_t_v7")
    async def rename_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(RenameTicketModal())

    @discord.ui.button(label="➕ הוסף משתמש", style=discord.ButtonStyle.secondary, custom_id="btn_add_m_t_v7")
...
class TicketDropdown(discord.ui.Select):
    def __init__(self):
        ...
        super().__init__(placeholder="🔽 בחר את קטגוריית הפנייה שלך...", options=options, custom_id="ticket_dropdown_v7_auto")
...
class GiveawayPanelView(discord.ui.View):
    ...
    @discord.ui.button(label="🎁 פתח הגרלה חדשה לשחקנים", style=discord.ButtonStyle.green, custom_id="btn_g_v7_auto")
...
class WarnPanelView(discord.ui.View):
    def __init__(self): super().__init__(timeout=None)
    @discord.ui.button(label="⚠️ רשום אזהרה למנהל", style=discord.ButtonStyle.danger, custom_id="btn_w_v7_auto")
...
class SuggestionsPanelView(discord.ui.View):
    def __init__(self): super().__init__(timeout=None)
    @discord.ui.button(label="🗳️ לחצו כאן והגישו הצעה חדשה לעיר", style=discord.ButtonStyle.primary, custom_id="btn_s_v7_auto")
    async def open_s(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.guild.get_role(VERIFY_ROLE_ID) not in interaction.user.roles and not interaction.user.guild_permissions.administrator:
            return await interaction.response.send_message("❌ תתאמת קודם!", ephemeral=True)
...
async def run_automatic_setup():
    ...
    if v_ch:
        ...
        await v_ch.send(embed=discord.Embed(title="🔐 מערכת אימות | CHICAGO CITY", description="לחץ על כפתור האימות מטה לקבלת גישה מלאה לכל ערוצי השרת.", color=0x2ecc71).set_thumbnail(url=LOGO_URL), view=VerifyView())
    ...
    if t_ch:
        ...
        await t_ch.send(embed=discord.Embed(title="🎫 מרכז תמיכה ופניות | CHICAGO CITY", description="צריך עזרה או ערוץ פנייה פרטי? בחר את הקטגוריה המתאימה בתפריט הבחירה מטה.", color=0x3498db).set_thumbnail(url=LOGO_URL), view=TicketOpenView())
    ...
    if g_ch:
        ...
        await g_ch.send(embed=discord.Embed(title="🎁 מרכז ניהול הגרלות השחקנים של הצוות", description="לחצו למטה לפתיחת טופס ההגרלה המהיר!", color=0x2ecc71).set_thumbnail(url=LOGO_URL), view=GiveawayPanelView())
    ...
    if w_ch:
        ...
        await w_ch.send(embed=discord.Embed(title="⚠️ פנל פיקוח ומשמעת הצוות", description="מרכז שליטה חסוי לרישום משמעת בצוות השרת.\n\n**🚨 רק דרג ניהול עליון מורשה ללחוץ!**", color=0xe67e22).set_thumbnail(url=LOGO_URL), view=WarnPanelView())
    ...
    if s_ch:
        ...
        await s_ch.send(embed=discord.Embed(title="💎 תיבת הרעיונות וההצעות של CHICAGO CITY", description="לחצו למטה, מלאו את הטופס וההצעה שלכם עולה ישירות לקהילה!", color=0xf1c40f).set_thumbnail(url=LOGO_URL), view=SuggestionsPanelView())
...
async def on_ready():
    ...
    # מפעיל את פונקציית ההקמה האוטומטית ברקע ברגע שהבוט מתחבר
